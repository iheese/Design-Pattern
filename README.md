...
### Bridge
- 구현부와 추상부를 분리하여 각자 독립적으로 변형, 개발 가능하게 한다. 
- OCP, SRP 를 만족한다.

#### 예제 설명
1. Shape 인터페이스가 있고, 그것의 구현체인 Circle, Square 이 있다.
2. Painter 클래스가 있고, 해당 클래스는 shape 타입 객체를 주입받아 메소드를 실행한다. 
3. Painter 구현체는 Picasso, VanGogh 가 있으며, 이 역시 Shape 타입 객체를 주입받아 메소드를 실행한다.
4. 원하는 모양 구체를 화가 객체에 주입하여 메소드를 실행시킨다. 
